...
const BASE_SPLITS = [
  { id: "breakfast", label: "Breakfast", icon: "", split: { calories: 0.206, protein: 0.201, carbs: 0.235, fat: 0.140 } },
...
// ─── Design tokens ────────────────────────────────────────────────────────────
const C = {
  bg: "#CEE7F3",           // pale cerulean — main background
  bgCard: "#daeef7",       // slightly lighter cerulean for cards
  bgWarm: "#c2dfec",       // deeper cerulean for inputs/surfaces
  bgSand: "#3C5070",       // royal blue — secondary surfaces
  text: "#A91B18",         // tuscan red — primary text
  textMid: "#8a1614",      // deeper tuscan red — secondary text
  textLight: "#3C5070",    // royal blue — labels and secondary info
  accent: "#F4ECD8",       // satin linen — buttons, active states, nourish. dot
  accentWarm: "#E0C58F",   // quicksand — secondary accent
  accentSoft: "#C9CABD",   // pale silver — subtle
  good: "#B46C72",         // rose gold — on track
  under: "#E0C58F",        // quicksand — off track/under
  over: "#E0C58F",         // quicksand — over
  border: "rgba(60,80,112,0.2)",
  borderMid: "rgba(60,80,112,0.35)",
};
...
const UNITS = [
  { label: "g",    toGrams: v => v },
  { label: "kg",   toGrams: v => v * 1000 },
  { label: "oz",   toGrams: v => v * 28.3495 },
  { label: "lb",   toGrams: v => v * 453.592 },
  { label: "cup",  toGrams: v => v * 240 },
  { label: "½ cup",toGrams: v => v * 120 },
  { label: "tbsp", toGrams: v => v * 15 },
  { label: "tsp",  toGrams: v => v * 5 },
  { label: "ml",   toGrams: v => v },
  { label: "L",    toGrams: v => v * 1000 },
  { label: "slice",toGrams: v => v * 30 },
  { label: "piece",toGrams: v => v * 50 },
];
...
async function lookupBarcode(barcode) {
  ...
  const cal = parseFloat(n["energy-kcal_100g"]) || parseFloat(n["energy-kcal_serving"]) || 0;
  if (!cal) throw new Error("No nutrition data for this product");
  return {
    id: "barcode_" + barcode,
...
function BarcodeScanner({ onResult, onClose }) {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (barcodes.length > 0) {
          ...
          try {
            ...
            setError(`Barcode found (${barcode}) but product not in database. Try searching manually.`);
            setStatus("error");
          }
          return;
        }
    ...
    start();
    return () => { if (rafRef.current) clearTimeout(rafRef.current); if (streamRef.current) streamRef.current.getTracks().forEach(t => t.stop()); };
  }, []);
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.92)", zIndex: 400, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
      <div style={{ position: "relative", width: "100%", maxWidth: 420 }}>
        ...
        <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", pointerEvents: "none" }}>
          <div style={{ width: 240, height: 140, border: "2px solid #F4ECD8", borderRadius: 12, boxShadow: "0 0 0 1000px rgba(0,0,0,0.45)" }}>
            <div style={{ position: "absolute", top: 0, left: 0, width: 24, height: 24, borderTop: "3px solid #F4ECD8", borderLeft: "3px solid #F4ECD8", borderRadius: "4px 0 0 0" }} />
            <div style={{ position: "absolute", top: 0, right: 0, width: 24, height: 24, borderTop: "3px solid #F4ECD8", borderRight: "3px solid #F4ECD8", borderRadius: "0 4px 0 0" }} />
            <div style={{ position: "absolute", bottom: 0, left: 0, width: 24, height: 24, borderBottom: "3px solid #F4ECD8", borderLeft: "3px solid #F4ECD8", borderRadius: "0 0 0 4px" }} />
            <div style={{ position: "absolute", bottom: 0, right: 0, width: 24, height: 24, borderBottom: "3px solid #F4ECD8", borderRight: "3px solid #F4ECD8", borderRadius: "0 0 4px 0" }} />
          </div>
        </div>
      </div>
      <div style={{ marginTop: 24, textAlign: "center", padding: "0 24px" }}>
        {status === "scanning" && <p style={{ color: "#F4ECD8", fontFamily: "'DM Sans', sans-serif", fontSize: 14 }}>Point camera at a barcode</p>}
        {status === "found" && <p style={{ color: "#B46C72", fontFamily: "'DM Sans', sans-serif", fontSize: 14 }}>Barcode found! Looking up product…</p>}
        {status === "error" && <p style={{ color: "#E0C58F", fontFamily: "'DM Sans', sans-serif", fontSize: 13, lineHeight: 1.5 }}>{error}</p>}
        {status === "starting" && <p style={{ color: "#F4ECD8", fontFamily: "'DM Sans', sans-serif", fontSize: 14 }}>Starting camera…</p>}
      </div>
      <button onClick={onClose} style={{ marginTop: 16, background: "rgba(244,236,216,0.15)", border: "1px solid rgba(244,236,216,0.3)", color: "#F4ECD8", borderRadius: 12, padding: "10px 28px", fontSize: 14, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>Cancel</button>
    </div>
  );
}
...
function AmountModal({ food, onConfirm, onClose, onSaveToLibrary }) {
  ...
  const handleConfirm = () => {
    ...
    const entry = {
      ...food,
      id: food.id.toString().startsWith("cn_") || food.id.toString().startsWith("barcode_") ? newId() : food.id,
  ...
  // Quick qty presets depending on unit
  const QUICK_QTY = unit === "g" ? ["50","75","100","150","200","300"] :
                    unit === "oz" ? ["1","2","3","4","5","6"] :
                    unit === "cup" ? ["0.25","0.5","0.75","1","1.5","2"] :
                    unit === "tbsp" ? ["1","2","3","4"] :
                    unit === "tsp" ? ["1","2","3"] :
                    ["1","2","3","4"];

  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(169,27,24,0.18)", zIndex: 300, display: "flex", alignItems: "flex-end", justifyContent: "center", backdropFilter: "blur(6px)" }} onClick={e => e.target === e.currentTarget && onClose()}>
      <div style={{ width: "100%", maxWidth: 480, background: C.bg, borderRadius: "24px 24px 0 0", padding: "24px 24px 32px", boxShadow: "0 -8px 40px rgba(169,27,24,0.12)" }}>

        {/* Header */}
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
          <div style={{ flex: 1, paddingRight: 12 }}>
            <div style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 18, color: C.text, lineHeight: 1.3, marginBottom: 4 }}>{food.name}</div>
            <div style={{ fontSize: 11, color: C.textLight, fontFamily: "'DM Sans', sans-serif" }}>
              {food.per100g ? `${Math.round(food.per100g.calories)} cal · ${food.per100g.protein?.toFixed(1)}P · ${food.per100g.carbs?.toFixed(1)}C · ${food.per100g.fat?.toFixed(1)}F per 100g` : `${food.serving} · ${food.calories} cal`}
            </div>
          </div>
          <button onClick={onClose} style={{ background: C.bgWarm, border: "none", color: C.textMid, borderRadius: 10, width: 32, height: 32, cursor: "pointer", fontSize: 16, flexShrink: 0 }}>✕</button>
        </div>

        {/* Amount: number + unit dropdown */}
        <div style={{ fontSize: 12, color: C.textLight, marginBottom: 8, fontFamily: "'DM Sans', sans-serif" }}>How much did you have?</div>
        <div style={{ display: "flex", gap: 10, marginBottom: 14 }}>
          <input
            ...
            min="0"
            step="any"
            style={{ ...INP, flex: 1, fontSize: 22, padding: "13px 16px", fontFamily: "'Playfair Display', Georgia, serif", fontWeight: 600, color: C.text, textAlign: "center" }}
          />
          <select
            value={unit}
            onChange={e => setUnit(e.target.value)}
            style={{ background: C.bgWarm, border: `1px solid ${C.border}`, borderRadius: 12, padding: "13px 14px", fontSize: 15, color: C.text, outline: "none", fontFamily: "'DM Sans', sans-serif", minWidth: 90, cursor: "pointer" }}
          >
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 18 }}>
          {QUICK_QTY.map(q => (
            <button key={q} onClick={() => setQty(q)} style={{ background: qty === q ? C.accent : C.bgWarm, border: `1px solid ${qty === q ? C.border : C.border}`, color: qty === q ? C.text : C.textMid, borderRadius: 20, padding: "6px 14px", fontSize: 13, cursor: "pointer", fontFamily: "'DM Sans', sans-serif", transition: "all 0.15s" }}>{q} {unit}</button>
          ))}
        </div>
        ...
        {computed ? (
          <div style={{ background: C.bgWarm, borderRadius: 14, padding: "14px 16px", marginBottom: 18, border: `1px solid ${C.border}` }}>
            <div style={{ fontSize: 10, color: C.textLight, letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 10, fontFamily: "'DM Sans', sans-serif" }}>Macros for {qty} {unit}{grams ? ` (≈${Math.round(grams)}g)` : ""}</div>
            <div style={{ display: "flex", gap: 10, justifyContent: "space-around" }}>
              {[["Cal", computed.calories, ""],["Prot", computed.protein, "g"],["Carb", computed.carbs, "g"],["Fat", computed.fat, "g"]].map(([lbl, val, u]) => (
                <div key={lbl} style={{ textAlign: "center" }}>
                  <div style={{ fontSize: 10, color: C.textLight, fontFamily: "'DM Sans', sans-serif", marginBottom: 3 }}>{lbl}</div>
                  <div style={{ fontSize: 22, fontWeight: 500, color: C.text, fontFamily: "'Playfair Display', Georgia, serif", fontStyle: "italic", lineHeight: 1 }}>{val}<span style={{ fontSize: 11, color: C.textLight, fontWeight: 400, fontStyle: "normal" }}>{u}</span></div>
                </div>
              ))}
            </div>
          </div>
        ...
        {food.fromDatabase && (
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 18, cursor: "pointer" }} onClick={() => setSaveToLib(v => !v)}>
            <div style={{ width: 22, height: 22, borderRadius: 6, border: `1.5px solid ${saveToLib ? C.text : C.border}`, background: saveToLib ? C.accent : "transparent", display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.2s", flexShrink: 0 }}>
              {saveToLib && <span style={{ color: C.text, fontSize: 13, lineHeight: 1 }}>✓</span>}
            </div>
        ...
        <button
          ...
          disabled={!computed}
          style={{ width: "100%", background: computed ? C.accent : C.bgWarm, border: `1.5px solid ${C.border}`, color: computed ? C.text : C.textLight, borderRadius: 14, padding: "14px", fontSize: 16, fontWeight: 600, cursor: computed ? "pointer" : "default", fontFamily: "'DM Sans', sans-serif", transition: "all 0.2s" }}
        >
...
function MacroPill({ label, actual, target }) {
  ...
  const color = st === "good" ? "#B46C72" : st === "empty" ? C.textLight : "#b89a5a";
  const bg = st === "good" ? "rgba(180,108,114,0.15)" : st === "empty" ? "rgba(60,80,112,0.06)" : "rgba(224,197,143,0.2)";
  const border = logged ? color + "50" : C.border;
  return (
...
function TotalBar({ label, value, goal }) {
  ...
  const barColor = good ? "#B46C72" : value > 0 ? "#b89a5a" : C.accentSoft;
  return (
    <div style={{ flex: 1 }}>
  ...
  return (
    <div style={{ display: "flex", gap: 2, justifyContent: "center", marginTop: 3 }}>
      {macros.map(m => {
        ...
        return <div key={m.key} style={{ width: 4, height: 4, borderRadius: "50%", background: st === "good" ? "#B46C72" : st === "empty" ? C.textLight : "#b89a5a" }} />;
      })}
    </div>
  );
}
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(61,53,48,0.5)", zIndex: 100, display: "flex", alignItems: "flex-end", justifyContent: "center", backdropFilter: "blur(4px)" }}>
      <div style={{ width: "100%", maxWidth: 600, background: C.bg, borderRadius: "24px 24px 0 0", maxHeight: "92dvh", display: "flex", flexDirection: "column", boxShadow: "0 -8px 40px rgba(100,80,60,0.15)" }}>
        ...
        <div style={{ padding: "16px 24px", borderTop: `1px solid ${C.border}`, flexShrink: 0 }}>
          ...
          <button onClick={handleSave} disabled={!name} style={{ width: "100%", background: name ? C.accent : C.bgSand, border: "none", color: name ? C.text : C.textLight, borderRadius: 14, padding: "14px", fontSize: 15, fontWeight: 600, cursor: name ? "pointer" : "default", fontFamily: "inherit", transition: "all 0.2s" }}>
...
// ─── Intuition Modal ──────────────────────────────────────────────────────────
function IntuitionModal({ slot, target, foods, slots, onAdd, onClose }) {
  ...
  useEffect(() => {
    async function generate() {
      try {
        // Build history meals from localStorage
        const historyMeals = [];
        for (let i = 0; i < localStorage.length; i++) {
          const key = localStorage.key(i);
          if (!key?.startsWith("md_slots_")) continue;
          try {
            const data = JSON.parse(localStorage.getItem(key));
            if (data?.[slot.id]?.length > 0) {
              const items = data[slot.id];
              ...
              historyMeals.push({
                date: key.replace("md_slots_", ""),
                items: items.map(i => `${i.food.name} (${i.servings} serving, ${Math.round(i.food.calories * i.servings)}cal)`).join(", "),
                totals,
              });
            }
          } catch {}
        ...
        const libraryList = foods.slice(0, 60).map(f =>
          `${f.name}: ${f.calories}cal, ${f.protein}P, ${f.carbs}C, ${f.fat}F per serving`
        ).join("\n");

        const historyList = historyMeals.slice(-14).map(m =>
          `${m.date} ${slot.label}: ${m.items} → ${Math.round(m.totals.calories)}cal, ${Math.round(m.totals.protein)}P, ${Math.round(m.totals.carbs)}C, ${Math.round(m.totals.fat)}F`
        ).join("\n");

        const prompt = `You are a macro-aware meal assistant. The user wants suggestions for ${slot.label}.
...
Their food library:
${libraryList || "No library items yet."}

Their recent ${slot.label} history:
...
[
  {
    ...
    "description": "Short note on why it fits",
    "items": [
      { "name": "Food name", "servings": 1, "calories": 300, "protein": 25, "carbs": 30, "fat": 8 }
    ],
    "totals": { "calories": 300, "protein": 25, "carbs": 30, "fat": 8 }
  }
]

Rules:
- Return 2-3 history meal suggestions first (type: "history"), pulled from their past ${slot.label} meals
- Then 2-3 individual food suggestions (type: "food") from their library that fit well
- Match macros as closely as possible — within 15% of each target
- Use realistic serving sizes from their history
- Only suggest foods that exist in their library or history
- Keep titles short and natural (e.g. "Chicken & Rice Bowl", "Your Thursday Dinner")
- Keep descriptions to one sentence max`;
        ...
        const text = data.content?.filter(b => b.type === "text").map(b => b.text).join("") || "";
        const match = text.replace(/```json\n?|\n?```/g, "").match(/\[[\s\S]*\]/);
        if (!match) throw new Error("No suggestions");
  ...
  const handleAdd = (suggestion) => {
    if (suggestion.type === "history" && suggestion.items) {
      suggestion.items.forEach(item => {
        const libraryFood = foods.find(f => f.name.toLowerCase() === item.name.toLowerCase());
        const food = libraryFood || {
          id: newId(), name: item.name,
          calories: item.calories, protein: item.protein,
          carbs: item.carbs, fat: item.fat,
          serving: `${item.servings} serving`,
        };
        onAdd({ ...food, calories: item.calories, protein: item.protein, carbs: item.carbs, fat: item.fat });
      });
    } else if (suggestion.items?.[0]) {
      const item = suggestion.items[0];
      const libraryFood = foods.find(f => f.name.toLowerCase() === item.name.toLowerCase());
      const food = libraryFood || { id: newId(), name: item.name, calories: item.calories, protein: item.protein, carbs: item.carbs, fat: item.fat, serving: "1 serving" };
      onAdd({ ...food, calories: item.calories, protein: item.protein, carbs: item.carbs, fat: item.fat });
    }
    setAdded(prev => new Set([...prev, suggestion.title]));
  };

  const MacroMatch = ({ totals }) => {
    const calPct = Math.round((totals.calories / target.calories) * 100);
    const color = calPct >= 85 && calPct <= 115 ? "#B46C72" : "#b89a5a";
    return (
      <div style={{ display: "flex", gap: 10, marginTop: 6, flexWrap: "wrap" }}>
        {[["Cal", totals.calories, target.calories, ""],["P", totals.protein, target.protein, "g"],["C", totals.carbs, target.carbs, "g"],["F", totals.fat, target.fat, "g"]].map(([lbl, val, tgt, u]) => {
          const pct = Math.round((val / tgt) * 100);
          const c = pct >= 85 && pct <= 115 ? "#B46C72" : "#b89a5a";
          return (
            <div key={lbl} style={{ display: "flex", alignItems: "center", gap: 3 }}>
              <span style={{ fontSize: 10, color: C.textLight, fontFamily: "'DM Sans', sans-serif" }}>{lbl}</span>
              <span style={{ fontSize: 12, color: c, fontFamily: "'Playfair Display', serif", fontStyle: "italic", fontWeight: 600 }}>{Math.round(val)}{u}</span>
              <span style={{ fontSize: 9, color: c, fontFamily: "'DM Sans', sans-serif" }}>({pct}%)</span>
            </div>
          );
        })}
      </div>
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(169,27,24,0.18)", zIndex: 300, display: "flex", alignItems: "flex-end", justifyContent: "center", backdropFilter: "blur(6px)" }} onClick={e => e.target === e.currentTarget && onClose()}>
      <div style={{ width: "100%", maxWidth: 520, background: C.bg, borderRadius: "24px 24px 0 0", maxHeight: "88dvh", display: "flex", flexDirection: "column", boxShadow: "0 -8px 40px rgba(169,27,24,0.14)" }}>

        {/* Header */}
        <div style={{ padding: "22px 24px 16px", borderBottom: `1px solid ${C.border}`, flexShrink: 0 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <div>
              <div style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 22, fontStyle: "italic", color: C.text, marginBottom: 3 }}>Intuition</div>
              <div style={{ fontSize: 12, color: C.textLight, fontFamily: "'DM Sans', sans-serif" }}>
                {slot.label} · targeting {target.calories} cal · {target.protein}P · {target.carbs}C · {target.fat}F
              </div>
            </div>
            <button onClick={onClose} style={{ background: C.bgWarm, border: "none", color: C.textMid, borderRadius: 10, width: 32, height: 32, cursor: "pointer", fontSize: 16, flexShrink: 0 }}>✕</button>
          </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 24px 24px" }}>
          {loading && (
            <div style={{ textAlign: "center", padding: "40px 0" }}>
              <div style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 16, fontStyle: "italic", color: C.textMid, marginBottom: 8 }}>Finding what fits…</div>
              <div style={{ fontSize: 12, color: C.textLight, fontFamily: "'DM Sans', sans-serif" }}>Analysing your history and library</div>
            </div>
          )}

          {!loading && suggestions?.length === 0 && (
            <div style={{ textAlign: "center", padding: "40px 0" }}>
              <div style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: 16, color: C.textMid, marginBottom: 8 }}>Not enough history yet</div>
              <div style={{ fontSize: 13, color: C.textLight, lineHeight: 1.6 }}>Log a few more meals and Intuition will start recognising your patterns.</div>
            </div>
          )}
            ...
            <>
              ...
              {suggestions.filter(s => s.type === "history").length > 0 && (
                <div style={{ marginBottom: 20 }}>
                  <div style={{ fontSize: 10, color: C.textLight, letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 12, fontFamily: "'DM Sans', sans-serif" }}>From your history</div>
                  {suggestions.filter(s => s.type === "history").map((s, i) => (
                    <div key={i} style={{ background: C.bgCard, borderRadius: 14, padding: "14px 16px", marginBottom: 10, border: `1px solid ${C.border}` }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10 }}>
                        <div style={{ flex: 1 }}>
                          <div style={{ fontSize: 15, fontWeight: 600, color: C.text, fontFamily: "'Playfair Display', Georgia, serif", marginBottom: 3 }}>{s.title}</div>
                          <div style={{ fontSize: 12, color: C.textLight, fontFamily: "'DM Sans', sans-serif", marginBottom: 6 }}>{s.description}</div>
                          <div style={{ fontSize: 11, color: C.textMid, fontFamily: "'DM Sans', sans-serif" }}>
                            {s.items?.map(i => i.name).join(" · ")}
                          </div>
                          <MacroMatch totals={s.totals} />
                        </div>
                        <button
                          onClick={() => handleAdd(s)}
                          style={{ background: added.has(s.title) ? C.bgWarm : C.text, border: "none", color: added.has(s.title) ? C.textLight : C.accent, borderRadius: 10, padding: "8px 16px", fontSize: 12, fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap", flexShrink: 0, fontFamily: "'DM Sans', sans-serif" }}
                        >{added.has(s.title) ? "Added ✓" : "Add all"}</button>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {/* Individual food suggestions */}
              {suggestions.filter(s => s.type === "food").length > 0 && (
                <div>
                  <div style={{ fontSize: 10, color: C.textLight, letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 12, fontFamily: "'DM Sans', sans-serif" }}>From your library</div>
                  {suggestions.filter(s => s.type === "food").map((s, i) => (
                    <div key={i} style={{ background: C.bgCard, borderRadius: 14, padding: "14px 16px", marginBottom: 10, border: `1px solid ${C.border}` }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10 }}>
                        <div style={{ flex: 1 }}>
                          <div style={{ fontSize: 15, fontWeight: 600, color: C.text, fontFamily: "'Playfair Display', Georgia, serif", marginBottom: 3 }}>{s.title}</div>
                          <div style={{ fontSize: 12, color: C.textLight, fontFamily: "'DM Sans', sans-serif", marginBottom: 4 }}>{s.description}</div>
                          <MacroMatch totals={s.totals} />
                        </div>
                        <button
                          onClick={() => handleAdd(s)}
                          style={{ background: added.has(s.title) ? C.bgWarm : C.text, border: "none", color: added.has(s.title) ? C.textLight : C.accent, borderRadius: 10, padding: "8px 16px", fontSize: 12, fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap", flexShrink: 0, fontFamily: "'DM Sans', sans-serif" }}
                        >{added.has(s.title) ? "Added ✓" : "Add"}</button>
                      </div>
                    </div>
                  ))}
                </div>
      ...
      <div style={{ width: "100%", maxWidth: 480, background: C.bg, borderRadius: "24px 24px 0 0", padding: "24px 24px 36px", boxShadow: "0 -8px 40px rgba(100,80,60,0.15)" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: C.textLight, marginTop: 3 }}>{slot.label} · type in your macros directly</div>
          </div>
        ...
        <button
          onClick={handleAdd}
          disabled={!valid}
          style={{ width: "100%", background: valid ? C.accent : C.bgSand, border: "none", color: valid ? C.text : C.textLight, borderRadius: 14, padding: "14px", fontSize: 16, fontWeight: 600, cursor: valid ? "pointer" : "default", fontFamily: "'DM Sans', sans-serif", transition: "all 0.2s" }}
        >
      ...
      <div style={{ width: "100%", maxWidth: 480, background: C.bg, borderRadius: "24px 24px 0 0", padding: "24px 24px 36px", boxShadow: "0 -8px 40px rgba(100,80,60,0.15)" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: C.textLight, fontFamily: "'DM Sans', sans-serif", marginTop: 3 }}>{slot.label} · {items.length} item{items.length !== 1 ? "s" : ""}</div>
          </div>
          <button onClick={onClose} style={{ background: C.bgSand, border: "none", color: C.textMid, borderRadius: 10, width: 32, height: 32, cursor: "pointer", fontSize: 16 }}>✕</button>
        </div>
        ...
        <button
          onClick={handleSave}
          disabled={!name.trim()}
          style={{ width: "100%", background: name.trim() ? C.accent : C.bgSand, border: "none", color: name.trim() ? C.text : C.textLight, borderRadius: 14, padding: "14px", fontSize: 16, fontWeight: 600, cursor: name.trim() ? "pointer" : "default", fontFamily: "'DM Sans', sans-serif", transition: "all 0.2s" }}
        >
          Save to Library
        </button>
      </div>
      ...
      <div style={{ padding: "14px 20px", background: C.bgWarm, borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        {editingPct ? (
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <button onClick={savePct} style={{ background: C.accent, border: `1.5px solid ${C.text}`, color: C.text, borderRadius: 10, padding: "7px 14px", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>Save</button>
          </div>
        ) : (
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
...
function GoalsView({ goals, targetPct, onUpdateGoals, onUpdateTargetPct }) {
  ...
  return (
    <div style={{ padding: "20px" }}>
      ...
      <button
        onClick={saveAll}
        style={{ width: "100%", background: saved ? C.good : C.accent, border: "none", color: C.text, borderRadius: 14, padding: "15px", fontSize: 16, fontWeight: 600, cursor: "pointer", fontFamily: "'DM Sans', sans-serif", transition: "all 0.3s" }}
      >
...
export default function App() {
  ...
  const [intuitionSlot, setIntuitionSlot] = useState(null);
  const [scanResult, setScanResult] = useState(null);
  const lastScanSlot = useRef(null);
  ...
  return (
    <div style={{ height: "100dvh", background: C.bg, color: C.text, fontFamily: "'Lora', Georgia, serif", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400;0,600;0,700;1,400;1,600&family=Lora:ital,wght@0,400;0,500;0,600;0,700;1,400;1,500&family=DM+Sans:wght@300;400;500;600&display=swap');
      ...
      <div style={{ padding: "max(env(safe-area-inset-top), 12px) 20px 10px", borderBottom: `1px solid ${C.border}`, background: C.bgCard, display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
        <h1 style={{ margin: 0, fontFamily: "'Playfair Display', Georgia, serif", fontSize: 24, fontWeight: 700, color: C.text, letterSpacing: "-0.3px", lineHeight: 1 }}>
                  ...
                  {isOpen && (
                    <div style={{ padding: "0 20px 18px 62px" }}>
                      ...
                      <div style={{ display: "flex", gap: 8, marginTop: 6 }}>
                        ...
                        />
                        <button onClick={() => { lastScanSlot.current = slot.id; setScanningSlot(slot.id); }} style={{ background: C.accent, border: `1.5px solid ${C.text}`, color: C.text, borderRadius: 12, padding: "11px 13px", fontSize: 12, fontWeight: 700, cursor: "pointer", flexShrink: 0, fontFamily: "'DM Sans', sans-serif", whiteSpace: "nowrap" }}>Scan</button>
                      </div>
                      {/* Quick entry + New food + Recipes row */}
                      <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                        <button
                          onClick={() => setIntuitionSlot(slot.id)}
                          style={{ flex: 1, background: C.text, border: "none", color: C.accent, borderRadius: 12, padding: "10px 14px", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "'Playfair Display', Georgia, serif", fontStyle: "italic", letterSpacing: 0.3 }}
                        >Intuition</button>
                        <button
                          onClick={() => setQuickEntrySlot(slot.id)}
                          style={{ flex: 1, background: C.bgCard, border: `1px solid ${C.border}`, color: C.textMid, borderRadius: 12, padding: "10px 14px", fontSize: 13, fontWeight: 600, cursor: "pointer" }}
                        >Enter manually</button>
                        <button onClick={() => { setPendingSlot(slot.id); setView("recipes"); }} style={{ background: C.bgCard, border: `1px solid ${C.border}`, color: C.textMid, borderRadius: 12, padding: "10px 14px", fontSize: 13, fontWeight: 600, cursor: "pointer", whiteSpace: "nowrap" }}>Recipes</button>
                        <button
                          onClick={() => { setPendingSlot(slot.id); setShowAddFood(true); setView("library"); }}
                          style={{ background: C.bgCard, border: `1px solid ${C.border}`, color: C.textMid, borderRadius: 12, padding: "10px 14px", fontSize: 13, fontWeight: 600, cursor: "pointer", whiteSpace: "nowrap" }}
                        >+ New</button>
                      </div>
                    </div>
                  )}
      ...
      {view === "library" && (
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
          <div style={{ padding: "14px 20px", borderBottom: `1px solid ${C.border}`, display: "flex", gap: 10, flexShrink: 0, background: C.bgCard }}>
            ...
            <button onClick={() => setShowAddFood(v => !v)} style={{ background: C.accent, border: `1.5px solid ${C.text}`, color: C.text, borderRadius: 12, padding: "11px 20px", fontSize: 14, fontWeight: 600, cursor: "pointer", whiteSpace: "nowrap" }}>+ Add Food</button>
          </div>
          ...
          {showAddFood && (
            <div style={{ padding: "16px 20px", background: C.bgWarm, borderBottom: `1px solid ${C.border}`, flexShrink: 0 }}>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                <button onClick={saveFood} style={{ background: C.accent, border: `1.5px solid ${C.text}`, color: C.text, borderRadius: 12, padding: "11px 24px", fontSize: 14, fontWeight: 600, cursor: "pointer" }}>{pendingSlot ? "Save & Add to Meal" : "Save to Library"}</button>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
          <div style={{ padding: "14px 20px", borderBottom: `1px solid ${C.border}`, background: C.bgCard, flexShrink: 0 }}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              ...
              <button onClick={handleImport} disabled={importStatus === "loading" || !recipeUrl.trim()} style={{ background: importStatus === "loading" ? C.accentSoft : C.accent, border: "none", color: C.text, borderRadius: 12, padding: "11px 20px", fontSize: 14, fontWeight: 600, cursor: "pointer", minWidth: 90 }}>
                ...
                <div key={recipe.id} style={{ padding: "16px 20px", borderBottom: `1px solid ${C.border}` }}>
                  <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 14 }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 8, flexShrink: 0 }}>
                      {pendingSlot && <button onClick={() => { addRecipeToSlot(recipe, pendingSlot); setView("day"); setActiveSlot(pendingSlot); setPendingSlot(null); }} style={{ background: C.accent, border: `1.5px solid ${C.text}`, color: C.text, borderRadius: 10, padding: "8px 14px", fontSize: 12, fontWeight: 600, cursor: "pointer", whiteSpace: "nowrap" }}>+ Add to {BASE_SPLITS.find(s => s.id === pendingSlot)?.label}</button>}
                      <button onClick={() => deleteRecipe(recipe.id)} style={{ background: "none", border: "none", color: C.textLight, cursor: "pointer", fontSize: 16, textAlign: "center" }}> Remove</button>
                    </div>
                  </div>
      ...
      {intuitionSlot && (
        <IntuitionModal
          slot={BASE_SPLITS.find(s => s.id === intuitionSlot)}
          target={targets[intuitionSlot]}
          foods={foods}
          slots={slots}
          onAdd={(food) => { addToSlot(intuitionSlot, food); }}
          onClose={() => setIntuitionSlot(null)}
        />
      )}
      ...
      {scanningSlot && !scanResult && (
        <BarcodeScanner
          onResult={(food) => { setScanResult(food); setScanningSlot(null); }}
      ...
      {scanResult && (
        <AmountModal
          food={scanResult}
          onConfirm={(food) => {
            addToSlot(lastScanSlot.current || "breakfast", food);
            setScanResult(null);
          }}
      ...
      <div style={{ background: C.bgCard, borderTop: `1px solid ${C.border}`, display: "flex", flexShrink: 0, paddingBottom: "env(safe-area-inset-bottom, 0px)" }}>
        {TABS.map(([v, lbl]) => (
          <button key={v} onClick={() => setView(v)} style={{
            ...
            <div style={{ fontSize: 13, fontWeight: view === v ? 700 : 400, fontFamily: "'Playfair Display', Georgia, serif", color: view === v ? C.text : C.textLight, letterSpacing: 0.3, transition: "all 0.2s" }}>{lbl}</div>
            {view === v && <div style={{ width: 20, height: 1.5, background: C.text, borderRadius: 1 }} />}
          </button>
        ))}
      </div>
    </div>
