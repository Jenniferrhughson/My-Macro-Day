...
function MacroPill({ label, actual, target }) {
  ...
  const color = st === "good" ? C.good : st === "empty" ? C.textLight : C.under;
  const bg = st === "good" ? "rgba(138,171,137,0.12)" : st === "empty" ? C.bgWarm : "rgba(201,122,107,0.1)";
...
function TotalBar({ label, value, goal }) {
  ...
  const barColor = good ? C.good : value > 0 ? C.under : C.accentSoft;
  return (
    <div style={{ flex: 1 }}>
  ...
  return (
    <div style={{ display: "flex", gap: 2, justifyContent: "center", marginTop: 3 }}>
      {macros.map(m => {
        ...
        return <div key={m.key} style={{ width: 4, height: 4, borderRadius: "50%", background: st === "good" ? C.good : st === "empty" ? C.textLight : C.under }} />;
      })}
    </div>
  );
}
...
export default function App() {
  ...
  return (
    <div style={{ height: "100dvh", background: C.bg, color: C.text, fontFamily: "'Lora', Georgia, serif", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      {/* ── Header ── */}
      <div style={{ padding: "max(env(safe-area-inset-top), 16px) 20px 14px", borderBottom: `1px solid ${C.border}`, background: C.bgCard, display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
        <div>
          <h1 style={{ margin: 0, fontFamily: "'Lora', Georgia, serif", fontSize: 26, fontWeight: 600, color: C.text, letterSpacing: "-0.3px", lineHeight: 1 }}>
            nourish<span style={{ color: C.accent, fontStyle: "italic" }}>.</span>
          </h1>
          <div style={{ fontSize: 11, color: C.textLight, marginTop: 3, fontFamily: "'DM Sans', sans-serif", letterSpacing: 0.3 }}>
            {goals.calories} kcal · {goals.protein}g protein · {goals.carbs}g carbs · {goals.fat}g fat
          </div>
        </div>
        <div style={{ display: "flex", gap: 2, background: C.bgWarm, borderRadius: 22, padding: 4 }}>
          {TABS.map(([v, lbl]) => <button key={v} onClick={() => setView(v)} style={btnTab(v)}>{lbl}</button>)}
        </div>
      </div>
      ...
      <div style={{ height: "env(safe-area-inset-bottom, 0px)", flexShrink: 0, background: C.bg }} />
    </div>
  );
}
